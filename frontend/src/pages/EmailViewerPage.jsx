...
import { ArrowLeft, Flag, MousePointerClick, Eye, Loader2, ShieldCheck } from 'lucide-react';
...
export default function EmailViewerPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.get(`/phishing/emails/${id}`, { params: { userId: user.id } }),
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div style={{ maxWidth: '760px' }}>
          ...
          <div style={{
            ...
            padding: '5px 12px',
            background: email.isSafe ? 'rgba(16,185,129,0.08)' : 'rgba(245,158,11,0.08)',
            border: `1px solid ${email.isSafe ? 'rgba(16,185,129,0.25)' : 'rgba(245,158,11,0.25)'}`,
            borderRadius: '20px',
            fontSize: '11px', fontWeight: '700',
            color: email.isSafe ? '#10b981' : '#f59e0b',
            letterSpacing: '1px', marginBottom: '20px',
          }}>
            {email.isSafe ? '✓ LEGITIMATE EMAIL — TRAINING SCENARIO' : '⚠️ SIMULATION EMAIL — FOR TRAINING ONLY'}
          </div>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px' }}>
                ...
                <ActionBtn
                  icon={ShieldCheck}
                  label="Mark as Safe"
                  desc="Confirm this is legitimate"
                  color="#00d4ff"
                  loading={actionLoading === 'marked_safe'}
                  disabled={!!actionLoading}
